...
function sameSameButDifferent(num, maybeNum) {
  if (num === Number(num) && maybeNum === Number(maybeNum) && num === maybeNum) {
    return `"same, same"`
  } else if (num === Number(maybeNum)) {
...
function updateGrade(student, value) {
  student.grade = value;
  return `${student}`
}
...
function takeATicketNumber(line) {
  let currentTicket = line.push(1)

  return "Welcome. You are ticket number" + currentTicket
}
